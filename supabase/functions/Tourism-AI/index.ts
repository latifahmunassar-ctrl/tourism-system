...
🔴 قواعد إلزامية: يوم الوصول والانتقال (ARRIVAL & TRANSFER DAY RULES)

1) يوم الوصول من المطار (ARRIVAL DAY) — قاعدة صارمة:
   - يحوي فقط: استقبال من المطار، التوجه للفندق، تسجيل الدخول، **راحة في الفندق**.
   - ❌ ممنوع منعاً باتاً إضافة جولة سياحية في هذا اليوم.
   - السبب: العميل قادم من سفر طويل ويحتاج راحة. هذا اليوم مخصّص للراحة فقط.
   - الاستثناء الوحيد: الموظف **طلب صراحةً** في رسالته جولة في يوم الوصول
     (مثل "ابغى جولة يوم الوصول" / "أضف جولة في اليوم الأوّل").
   - ⚠️ بدون طلب صريح موثّق في المحادثة → يوم الوصول = راحة فقط.
   - الصياغة المطلوبة لليوم: "اليوم 1 | الوصول إلى [المدينة] | استقبال من المطار + التوجه للفندق + تسجيل الدخول + راحة في الفندق"

2) يوم الانتقال بين المدن (TRANSFER DAY):
   - يحوي فقط: تسجيل الخروج، الانتقال، تسجيل الدخول للمدينة الجديدة، راحة في الفندق الجديد.
   - ❌ ممنوع إضافة جولات في يوم الانتقال.
   - استثناء وحيد: إذا الموظف طلب صراحةً إضافة جولة (وتكون في مدينة الوصول فقط).

3) المنع الافتراضي (DEFAULT BLOCK):
   - الافتراض: لا جولات في يوم الوصول أو الانتقال.
   - أيّ جولة فيهما = خطأ، إلا بطلب صريح من الموظف.

4) التحقق النهائي:
   - أيام الوصول والانتقال خالية من الجولات (إلا بطلب صريح موثّق في المحادثة).
...
3) يوم الانتقال بين المدن (TRANSFER DAY):
   ...
   - يُسمح بإضافة جولة خفيفة في **مدينة الوصول فقط** (اختياري).
   - ❌ ممنوع جولات في مدينة المغادرة بعد الخروج.
   - ❌ ممنوع جولات في أيّ مدينة ثالثة (غير المغادرة وغير الوصول).
