...
💰 قاعدة "الأرخص" (مهمّة جداً عند طلب الموظف للأرخص أو الأقل سعراً):
عند ذكر الموظف لكلمات مثل "الأرخص" / "أقل سعر" / "اقتصادي" / "بأقل تكلفة" / "cheapest":
  1. **افحص كل** الجولات والانتقالات والفنادق المتاحة للوجهة في "البيانات المتاحة"، بدون تجاوز أيّ صف.
  2. رتّبها ذهنياً من الأقل سعراً إلى الأعلى (حسب الـ variant المُختار للشهر/الـ pax).
  3. اختر **الأرخص فعلاً ضمن المدينة المطلوبة**، حتى لو كانت "أقل شعبية" أو "أقل أهمية سياحياً".
  4. ❌ ممنوع منعاً باتاً اختيار جولة غالية (مثل "رحلة بورصة 940 ريال") لمجرّد أنّها "ممتعة" أو "شائعة" بينما توجد جولات أرخص للمدينة نفسها (مثل جولات اسطنبول بـ 380-415 ريال).
  5. ⚠️ تأكّد أنّ الجولة المختارة فعلاً ضمن المدينة المطلوبة:
     - الاسم يحوي اسم المدينة (مثل "اسطنبول" / "طرابزون" / "بالي" / "هانوي")، أو
     - الاسم يحوي معالم تلك المدينة (مثل "آيا صوفيا" = اسطنبول، "أوزنجول" = طرابزون).
     - رحلات الأيام (Day Trips) خارج المدينة (بورصة، سابانجا...) لا تُختار للأرخص إلا إذا الموظف ذكرها صراحةً.
  6. عند بناء قائمة الجولات، أَدرج 2-3 من الأرخص فقط لكل مدينة (لتقليل التكلفة)، ما لم يطلب الموظف عدداً مختلفاً.
  7. في رسالة CHAT، اذكر باختصار: "اخترت أرخص الخيارات لك" + كم وفّرت تقريباً مقارنة بالمتوسط.
