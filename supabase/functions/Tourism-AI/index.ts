...
const DEST_CITIES: Record<string, Array<{ canonical: string; pattern: RegExp }>> = {
  ...
  "South Africa": [
    // Cape Town + its day-trip attractions (all operated as day tours FROM Cape
...
✈️🌍 تعميم قاعدة الطيران الداخلي على **كل الوجهات** (فيتنام، تايلاند، وأي وجهة لها قسم "✈️ الطيران" في البيانات) — نفس منطق ماليزيا أعلاه حرفياً، مع هذه الإضافات الإلزامية:
   أ) **تسري حتى لو الطلب «بدون فنادق»:** استخرِج تسلسل المدن من **ترتيب أسطر HOTELS** إن وُجدت، **وإلا من توزيع المدن في الطلب** (مثال «2 كرابي، 5 بوكيت» أو «1 هانوي، 3 سابا، 3 دانانج، 1 هانوي») وأيام التنقّل. لا تعتمد على وجود الفنادق لتحديد الانتقالات.
   ب) لكل انتقال متتالٍ A→B: إن وُجد **صف انتقال برّي مباشر** A→B في البيانات (سيارة بين المدينتين، مثل هانوي↔سابا أو كرابي↔بوكيت) → استخدمه في TRANSFERS بلا طيران. وإلا، إن وُجدت **رحلة في قسم "✈️ الطيران" تربط مطار A بـ B** → ❌ ممنوع تمثيل الانتقال كـ«يوم تنقّل حر / 0 ريال»؛ بل فكّكه إلى الثلاث قطع كما في قاعدة ماليزيا: (Drop مغادرة لمطار A) + (سطر FLIGHTS طيران) + (Pickup استقبال مطار B) — **في يوم الانتقال نفسه، ذهاباً وعودةً بلا استثناء**.
   ج) **مدن بلا مطار:** إن كانت مدينة المغادرة A لا مطار فيها (مثل **سابا وهالونج في فيتنام مطارهما هانوي**) فقطعة المغادرة (Drop) تكون من A إلى **مطار هانوي**؛ استخدم صف الانتقال المطاري الموجود بالبيانات لتلك المدينة حرفياً إن وُجد (مثل «العوده من سابا الى هانوي … والتوجه للمطار الدولي»)، والطيران يقلع من هانوي.
   د) **لا تخترع:** إن نقص من البيانات فعلاً (صف انتقال المطار، أو الرحلة، أو الاستقبال) ⇒ أضِف المتوفّر واذكر النقص في CHAT، دون تلفيق سعر.
...
3) يوم الانتقال بين المدن (TRANSFER DAY):
   ...
   - ⛔🔴 **يوم انتقال جوّي (طيران بين المدينتين) — إلزام قاطع يتقدّم على كل ما سبق ويسري حتى «بدون فنادق»:** إذا كان الانتقال A→B **ليس** له صف سيارة برّي مباشر في البيانات، لكن يوجد في قسم «✈️ الطيران» رحلة تربط مطار A بـ B (مثل سابا/دانانج/هوتشي مينه في فيتنام، أو بوكيت/بانكوك في تايلاند):
     • ❌❌ **ممنوع منعاً باتاً** تمثيل هذا اليوم كسطر «يوم تنقّل من A الى B | حر | 0 ريال» في TOURS، وممنوع تركه نصّاً بلا محتوى.
     • ✅ **يجب** أن يحوي يوم الانتقال **ثلاثة سطور فعلية**: (١) سطر في **FLIGHTS**: «A - B | داخلي | السعر/شخص من الجدول ريال/شخص | عدد META أشخاص | الإجمالي ريال». (٢) سطر **TRANSFERS Drop**: توصيل من مدينة A إلى مطار إقلاعها. (٣) سطر **TRANSFERS Pickup**: الاستقبال من مطار B — كلها بنفس رقم اليوم.
     • **مدن بلا مطار:** سابا وهالونج (فيتنام) مطارهما **هانوي** — فقطعة Drop تكون من سابا/هالونج إلى مطار هانوي (استخدم صف الانتقال المطاري الموجود لتلك المدينة حرفياً إن وُجد)، والطيران يقلع من هانوي.
     • طبّقه على **كل** انتقالات الطيران بالبرنامج **ذهاباً وعودةً** (بما فيها العودة لمدينة القاعدة في آخر البرنامج). ❌ لا تخترع سعراً غير موجود؛ إن نقص فعلاً اذكره في CHAT.
...
function parseHaramTags(text: string): string[] {
  ...
  const viewRe = /(?:[اإ]طلال[ةه]?|مطل[ةه]?|تطل)\s*(?:عل[يى])?\s*/;   // إطلالة/مطلّة/تطل على …
  // «إطلالة على الكعبة» (أخصّ) — وهي منطقياً إطلالة على الحرم أيضاً.
  if (new RegExp(viewRe.source + "الكعب[ةه]").test(t)) { out.push("إطلالة على الكعبة"); out.push("إطلالة على الحرم"); }
  // «إطلالة على الحرم» أو صيغة الفعل «مطلّة/مطله على الحرم» (الغرفة تطل على الحرم).
  if (new RegExp(viewRe.source + "الحرم").test(t)) out.push("إطلالة على الحرم");
