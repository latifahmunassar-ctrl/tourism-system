...
Deno.serve(async (req) => {
  ...
  if (url.searchParams.get("admin_action") === "attendance_report") {
    ...
    try {
      ...
      const isAckText = (t: string | null): boolean => { const s = String(t || "").trim(); if (!s) return true; if (/[؟?]/.test(s) || /(^|[\s،])(كم|بكم|وكم|كام|متى|وين|فين|كيف|ليش|ليه|هل|ايش|أيش|وش|سعر|السعر|الاسعار|التكلفه|التكلفة)([\s،؟?]|$)/i.test(s)) return false; if (/^(نعم|ايوه|ايوا|أيوه|أيوا|ايوة|أيوة|اي|إي|اكيد|أكيد|اجل|أجل|تمام نعم|اي نعم|yes|yep|yeah)\.?$/i.test(s)) return false; if (/الغ[يا]|ألغ[يا]|نلغي|لغينا|نأجل|أجلنا|اجلنا|مأجل|مؤجل|ما ?راح ?نسافر|ما ?بنسافر|ما ?رح ?نسافر|غيرنا ?ر[اأ]ي|ما ?طاعوا|يرخصو|رخصونا|ما ?حصلنا ?[اإ]جاز|مو ?مناسب|ما ?يناسب|مايجي|ما ?يجي|مو ?الحين|مو ?هالفتر|نلغيها/.test(s)) return true; if (/ا?رد ?(لكم|عليكم|عليك|لكو)|اردلكم|راح ?ا?رد|بكلمكم|اكلمكم|أكلمكم|بخبركم|اخبركم|أخبركم|بفيدكم|افيدكم|أفيدكم|ارجعلكم|ارجع ?لكم|أرجع ?لكم|بتواصل ?معكم|اتواصل ?معكم|بعطيكم ?خبر|اعطيكم ?خبر|بشوف ?وارد|اشوف ?وارد|بشاور( |$)|اشاور( |$)/.test(s)) return true; if ([...s].length <= 6) return true; const stripped = s.replace(/(شكرا?ً?|مشكور(ين)?|تسلم(ون|وا)?|يسلمو|تسلمو|تشكر|جزا?ك(م)?|الله|يعطيك(م)?|العافيه|العافية|العاف|خير(ا|اً|ها)?|كثير(ا)?|جزيل(ا|اً)?|وافر|لك(م)?|الك|اخوي|أخوي|اختي|أختي|يا|غالي|حبيبي|عزيزي|تمام|تم|اوكي?|ok|okay|thanks|thx|ماشي|زين|طيب|ممتاز|رائع|روعه|حلو|كفو|مر(ه|ة)|جدا|جداً|واجد|والله|عاد|خلاص|تستاهل(ون)?|نورت|يحفظك|يسعدك|يوفقك|كل|على|عل)/gi, "").replace(/[\s،.!؛:؟?\-_()*👍🙏❤️🌹💚💐🤍😍🥰😘♥️🌟✨🤲🌷💕💜]+/g, ""); return [...stripped].length <= 3; };
